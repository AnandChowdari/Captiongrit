 ...
 * 
 * A self-contained Express server for Captiongrit plugin authentication.
...
// Helper: Generate a unique, formatted license key (e.g., CG-XXXX-XXXX-XXXX)
...
app.post('/api/activate', (req, res) => {
    const { email, licenseKey, plan } = req.body;
    ...
    let betaDaysLeft;
    if (plan === 'beta') {
        if (!license.betaFirstLogin) {
            license.betaFirstLogin = new Date().toISOString();
            writeDatabase(db);
        }
        const firstLoginTime = new Date(license.betaFirstLogin).getTime();
        const now = Date.now();
        const daysElapsed = (now - firstLoginTime) / (1000 * 60 * 60 * 24);
        if (daysElapsed > 7) {
            return res.status(403).json({ error: "Your 7-day Beta trial has expired. Please upgrade to a full license." });
        }
        betaDaysLeft = Math.max(0, Math.ceil(7 - daysElapsed));
    }

    // Generate a simple secure token for the plugin
    const token = crypto.createHash('sha256').update(cleanEmail + cleanKey + 'captiongrit_captions_salt_2026').digest('hex');
...
app.post('/api/admin/create-license', async (req, res) => {
    ...
    if (adminToken !== 'Bearer cg_admin_secret_token_123') {
    ...
    const mailOptions = {
        from: '"Captiongrit Support" <your-email@gmail.com>',
        to: cleanEmail,
        subject: '🐧 Your Captiongrit License Key & Credentials',
        html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
                <div style="text-align: center; border-bottom: 2px solid #00d4aa; padding-bottom: 12px; margin-bottom: 20px;">
                    <h1 style="color: #00d4aa; margin: 0; font-size: 24px;">Captiongrit</h1>
                    <p style="color: #666; margin: 5px 0 0 0;">AI Captions for After Effects & Premiere Pro</p>
                </div>
                
                <h2 style="color: #333; font-size: 18px;">Welcome to Captiongrit!</h2>
                ...
                <ol style="color: #555; line-height: 1.6; padding-left: 20px;">
                    ...
                    <li>Go to <strong>Window &rarr; Extensions &rarr; Captiongrit</strong>.</li>
                ...
                <p style="color: #888; font-size: 11px; margin-top: 30px; border-top: 1px solid #eaeaea; padding-top: 15px; text-align: center;">
                    If you have any questions or require support, please contact the Captiongrit Administrator.<br>
                    &copy; 2026 Captiongrit. All rights reserved.
                </p>
            </div>
        `
    };
...
app.listen(PORT, () => {
    console.log(`=================================================`);
    console.log(`Captiongrit License Server running on port ${PORT}`);
