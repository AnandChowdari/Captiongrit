...
function extractFunctionBody(funcName, nextFuncName) {
  let idx = code.indexOf('function ' + funcName + '(');
  let endIdx = nextFuncName ? code.indexOf('function ' + nextFuncName + '(', idx + 10) : code.indexOf('function ', idx + 10);
  if (endIdx === -1) endIdx = code.length;
  let funcStr = code.substring(idx, endIdx);
  return funcStr;
}

let handlePaidSignupStr = extractFunctionBody('handlePaidSignup', 'updateEmailStatusInSheetsLock');
let updateEmailLockStr = extractFunctionBody('updateEmailStatusInSheetsLock', 'updateEmailStatusInSheets');
let recStr = extractFunctionBody('recoverFailedPayment', 'validateDatabaseSchemas');
let execStr = extractFunctionBody('executeRazorpayRecoveries', 'handleAdminAction');
...
fs.writeFileSync('f:/Coding/Projects/Caption Integrit/genTest2.js', testCode, 'utf8');
